 ...
 * of Layer objects, with the topmost visible layer being the
 * one that the user would see and interact with.
 */
public interface MultiImage {

  /**
   * Adds a layer to the MultiImage. This layer by default goes behind
   * all the other layers so it is not visible unless the user
   * brings it to the front or sets the other layers to invisible.
   *
   ...
   * @throws IllegalArgumentException If the given name is not the
   *     name of a layer in the list
   */
  void bringToFront(String name) throws IllegalArgumentException;

  /**
   * Applies the given modification to the topmost, visible layer. If the top
   * layer is not visible, the modification will be applied to the next visible layer.
   *
  ...
  /**
   * Exports to a folder and saves as the given type.
   *
   * @param pathToFolder The path to the folder to save again
